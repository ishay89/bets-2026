# Publish Match Day from Pre-Seeded Draft — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite `/admin/publish` so the admin picks a date, the page loads the pre-seeded draft match day from the DB (teams + kickoff times pre-filled, odds editable), and publishing UPDATEs the existing draft rather than creating a new one.

**Architecture:** URL-param driven RSC — the date picker submits a GET form appending `?date=YYYY-MM-DD`. The page server-renders, queries the DB for a draft match day on that date, and pre-fills the form. The server action UPDATEs existing match rows' odds and sets `published_at` on the match day.

**Tech Stack:** Next.js 16 App Router (RSC + Server Actions), Supabase service client, TypeScript strict
...
| File | Change |
|------|--------|
| `app/admin/publish/page.tsx` | Full rewrite — new page component + new server action |

No migrations, no new files.

---

## Task 1: Rewrite `app/admin/publish/page.tsx`
...
The current page has two responsibilities: a `publishMatchDay` server action that **inserts** a brand-new match day + matches, and a `PublishPage` component that renders a blank form. Both need to change.

The seeded data is in `match_days` (with `published_at = null`) and `matches` (with teams + kickoff times + estimated odds). We need to:
1. Load that data by date (via `searchParams`)
2. Render it pre-filled
3. On submit: UPDATE match odds + set `published_at`

---

- [ ] **Step 1: Replace the server action**

Replace the entire contents of `app/admin/publish/page.tsx` with the following. This step covers the server action only — the page component comes in step 2 (but write the full file now).
...
Append the following to the same file (after the server action):
...
Expected: build completes with no TypeScript errors. If you see errors, they will be in `app/admin/publish/page.tsx` — fix types there before continuing.
...
Open `http://localhost:3000/admin/publish` and verify:
1. Page loads with date picker showing today's date
2. "Pick a date and click Load" hint is shown
3. Type a date that has seeded fixtures (e.g. `2026-06-11` — first match day) and click Load
4. Page reloads with matches shown: home/away team names read-only, kickoff time displayed, odds inputs pre-filled with seed values
5. Edit an odds value, add a Pikanteria question with yes/no odds, click Publish
6. You are redirected to `/admin/results`
7. In Supabase dashboard (or via `/admin/results`), confirm the match day now has `published_at` set and the match odds were updated

Also verify the "no draft" state:
8. Load a date with no fixtures (e.g. `2025-01-01`) — the amber notice appears

- [ ] **Step 5: Commit**

```bash
git add app/admin/publish/page.tsx
git commit -m "feat: publish page loads pre-seeded draft and updates odds"
```
