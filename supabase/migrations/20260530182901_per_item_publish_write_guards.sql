-- Mondial Bets 2026 — Per-item publishing: write-path guards
--
-- The save RPCs and the *_unlocked write policies from 012_atomic_prediction_saves
-- only gated on day-level match_days.published_at. With per-item publishing a
-- leaked draft-match/-pikanteria UUID would otherwise let a user pre-save against
-- an unpublished item. These recreate the RPCs and policies adding the per-item
-- published_at check.
