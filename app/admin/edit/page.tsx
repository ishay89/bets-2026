...
export default async function EditPage({
  ...
  type PublishedMatchDay = { id: string; stage: string; date: string; lock_time: string }
  type PublishedMatch = {
    id: string; home_team: string; away_team: string
    kickoff_time: string; odds_home: number; odds_draw: number; odds_away: number; result: string | null
  }
  ...
  if (date) {
    const { data: md } = await supabase
      .from('match_days')
      .select('id, stage, date, lock_time')
      .eq('date', date)
      .not('published_at', 'is', null)
      .maybeSingle()
    ...
    if (md) {
      ...
      const { data: matchRows } = await supabase
        .from('matches')
        .select('id, home_team, away_team, kickoff_time, odds_home, odds_draw, odds_away, result')
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-10">
      <div>
        ...
        <div className="text-muted text-xs">Update match odds for an already-published day</div>
      </div>
